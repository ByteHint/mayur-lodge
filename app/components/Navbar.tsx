...
export default function Navbar() {
  ...
  const scrollToSection = (id: string) => {
    ...
    if (elem) {
      ...
      setIsMenuOpen(false); // close on mobile scroll
    }
  };
    ...
    <div className="sticky top-0 z-50 bg-[#FFFFFF] text-[#3C3C3C] w-full mb-4 shadow-sm">
      <div className="relative flex items-center justify-between w-full mx-auto px-4 py-2 sm:px-6 md:px-10 lg:px-16 h-[72px]">
        ...
        <div className={`${aderoTrial.className} hidden md:flex justify-center flex-1 gap-10 text-sm font-semibold`}>
          ...
          <button onClick={() => scrollToSection('facilities')} className="hover:text-yellow-500 hover:scale-105 transition-all delay-150 duration-300 ease-in-out cursor-pointer uppercase">
            Facilities & Services
          </button>
          <button onClick={() => scrollToSection('gallery')} className="hover:text-yellow-500 hover:scale-105 transition-all delay-150 duration-300 ease-in-out cursor-pointer uppercase">
            Gallery
          </button>
          <button onClick={() => scrollToSection('location')} className="hover:text-yellow-500 hover:scale-105 transition-all delay-150 duration-300 ease-in-out cursor-pointer uppercase">
            Location
          </button>
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white px-6 py-4 flex flex-col gap-4 text-sm font-semibold border-t">
          <button onClick={() => scrollToSection('about')} className="uppercase" >About Us</button>
